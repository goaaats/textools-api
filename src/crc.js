const crc_table_0f085d0 = [0x00000000, 0x77073096,
...
const crc_table_0f089d0 = [0x00000000, 0x191B3141,
...
const crc_table_0f08dd0 = [0x00000000, 0x01C26A37,
...
const crc_table_0f091d0 = [0x00000000, 0xB8BC6765,
...
// Courtesy of FFXIVExplorer and RozeDoyanawa
module.exports = {
    GetHash: function (buf, offset, length) {
        var dwCRC = -1;
        var cbRunningLength = ((length < 4) ? 0 : (((length) / 4 | 0)) * 4);
        var cbUnalignedBytes = length - cbRunningLength;

        var bufPos = 0;

        for (var i = 0; i < cbRunningLength / 4; ++i) {
            dwCRC ^= buf.readInt32LE(bufPos);
            bufPos += 4;
        ...
        for (var i = 0; i < cbUnalignedBytes; ++i) {
            dwCRC = crc_table_0f085d0[(dwCRC ^ buf[bufPos]) & 0x000000FF] ^ (dwCRC >>> 8);
            bufPos++;
        }
